...
/**
 * Everything about a PDF that has to be true before we accept it, checked on
 * the bytes in hand.
 *
 * Deliberately separate from ingestFile, and called before the dedupe lookup.
 * Validation that runs after dedupe only ever sees bytes we have not stored
 * before, so a file that got in while a check was missing keeps being handed
 * back for as long as it exists — which is exactly what happened with the
 * first encrypted return: it was already stored, so re-uploading it returned
 * the old row and never reached the check meant to refuse it.
 *
 * Returns the page count, which the caller needs anyway.
 */
async function validatePdf(buffer: Buffer, filename: string): Promise<number> {
  let pdf: PDFDocument;
  ...
  try {
    ...
    // PDF apart from a broken one, which are different problems with different
    // remedies. The isEncrypted check below is what refuses it.
  ...
  // the load above succeeds and the page count is correct. The bytes then go to
  // the model, which cannot read them, and the answer comes back as "badly
  // formatted or corrupted" naming no file — so with several documents
  // attached, one locked file fails the whole message and the unlocked ones
  // take the blame. pdf-lib cannot decrypt (it has no password option at all),
  // so this is a refusal rather than something we can quietly fix.
  //
  // Worth saying in the message: most of these open with no password prompt. A
  // permissions lock encrypts the content while leaving the open password
...
export async function ingestFile(input: {
  ...
  // Before the dedupe lookup, not after: a file we have already stored must be
  // re-checked rather than handed straight back, or a check added later never
  // applies to anything already in the table.
  const pageCount = sniffed.kind === 'pdf' ? await validatePdf(buffer, filename) : null;
