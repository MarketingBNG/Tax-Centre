...
export function SeverityChip({ severity }: { severity: Severity | null }) {
  ...
  if (!severity) {
    return (
      <span className="inline-block rounded border border-line px-2 py-1 text-[11.5px] text-ink-faint">
        Agreed
      </span>
    );
  }
  return (
    <span
      className={`inline-block rounded border px-2 py-1 text-[11.5px] font-medium ${SEVERITY_STYLE[severity]}`}
    >
      {severity}
    </span>
  );
...
export function VerdictChip({ verdict }: { verdict: Verdict | null }) {
  ...
  return (
    <span
      className={`inline-block rounded border px-2 py-1 text-[11.5px] font-medium ${VERDICT_STYLE[verdict]}`}
    >
...
export function StatusChip({ status }: { status: FindingStatus }) {
  return (
    <span className={`inline-block rounded border px-2 py-1 text-[11.5px] ${STATUS_STYLE[status]}`}>
      {STATUS_LABEL[status]}
    </span>
  );
}
...
export function AuthorityChip({
  ...
  if (status === 'grounded' && citation) {
    return (
      <span className="inline-block rounded border border-verdict-clear/40 px-2 py-1 text-[11.5px] text-verdict-clear">
        {citation}
      </span>
    );
  }
  ...
  return (
    <span
      className="inline-block rounded border border-sev-high/40 px-2 py-1 text-[11.5px] text-sev-high"
      title={
        claimedCitation
