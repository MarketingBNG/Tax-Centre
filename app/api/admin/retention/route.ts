...
export async function GET() {
  ...
  const live = one<{ n: number; bytes: number }>(
    `SELECT COUNT(*) AS n, COALESCE(SUM(size_bytes),0) AS bytes
     FROM files WHERE deleted_at IS NULL`,
  );
  const due = one<{ n: number; bytes: number }>(
    `SELECT COUNT(*) AS n, COALESCE(SUM(size_bytes),0) AS bytes
     FROM files WHERE deleted_at IS NULL AND created_at < ?`,
    cutoff,
  );
  const purged = one<{ n: number }>(`SELECT COUNT(*) AS n FROM files WHERE deleted_at IS NOT NULL`);
...
export async function POST() {
  ...
  const result = sweepExpiredOriginals();
  audit(user.id, 'retention.sweep', 'files', null, result);
