...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const target = one<{ id: string; role: string; is_active: number; email: string }>(
  ...
  run(`UPDATE users SET is_active = ? WHERE id = ?`, makeActive ? 1 : 0, id);
  ...
  audit(user.id, 'user.set_active', 'user', id, { active: makeActive, email: target.email });
