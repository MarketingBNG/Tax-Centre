...
export async function GET() {
  ...
  return Response.json(
    all(`SELECT id, email, role, display_name, is_active, created_at FROM users ORDER BY created_at`),
  );
}
...
export async function POST(req: Request) {
  ...
  const created = createUser({ email, role, displayName: displayName || email });
  audit(user.id, 'user.create', 'user', created.id, { email: created.email, role });
